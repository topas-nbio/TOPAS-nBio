...
void DrUtils::PrintTypeTable(std::ofstream &file) {

    //@@@@ Print out column headers by starting with the simulation time then
    //@@@@ the molecule names in the order they appear in fMoleculesRecord
    //@@@@ followed by the number of unrepaired breaks at any time (counted as
    //@@@@ the number of BREAK ENDS.
    file << "#Time\t";
    auto titles = DrDefinitions::Instance()->fMoleculesRecord[1][0];
    for (auto read: titles){
        file << read.first->GetName() << "\t";
    }
    file << "Unrepaired" << G4endl;

    auto data = DrDefinitions::Instance()->fMoleculesRecord[1];
    G4double time{0.0};

    for (auto read : data){
        //@@@@ At each time point write out the time followed by the integer values
        //@@@@ of the molecules present in the simulation at that time in the order
        //@@@@ they appear in fMoleculesRecord
        time = read.first;
        file << time << "\t";
        for (auto read2: read.second){
            //@@@@ Itterate through the molecules
            G4int numberOfMolecules = read2.second[0];
            file << numberOfMolecules << "\t";
        }

        int repaired{0};

        for (auto read2: read.second){
            //@@@@ After raw values have been written out work out the number of
            //@@@@ of unrepaired molecules by going through the list again and
            //@@@@ finding DSB_Fixed. These are reported in terms of DSB ends.
            if(read2.first->GetName().substr(0,9) == "DSB_Fixed"){
                if(read2.first->GetName().substr(0,12) == "DSB_Fixed_HR"){
                    G4int numberOfMolecules = read2.second[0];
                    repaired += numberOfMolecules;
                }
                else{
                    G4int numberOfMolecules = read2.second[0]*2;
                    repaired += numberOfMolecules;
                }
            }
        }
        file << (DrDefinitions::Instance()->fInitialBreakNumber*2)-repaired << G4endl;
    }
