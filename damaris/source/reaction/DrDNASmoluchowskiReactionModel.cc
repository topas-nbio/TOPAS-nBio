...
G4bool DrDNASmoluchowskiReactionModel::FindReaction(const G4Track& trackA, const G4Track& trackB, const G4double reactionRange, G4double& separationDistance, const G4bool isAlongStepReaction){
    ...
    if (!doBreak){
        ...
        if(MoleculeAName.substr(0,3) == "DSB" && MoleculeBName.substr(0,3) == "DSB"){
            ...
            DrBreakMolecule* breakMoleculeA = (DrBreakMolecule*)(trackA.GetAuxiliaryTrackInformation(DrDefinitions::Instance()->GetBreakMoleculeIndex()));
            DrBreakMolecule* breakMoleculeB = (DrBreakMolecule*)(trackB.GetAuxiliaryTrackInformation(DrDefinitions::Instance()->GetBreakMoleculeIndex()));
