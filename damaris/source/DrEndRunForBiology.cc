    ...
    if(fPm->ParameterExists("Ch/DaMaRiS/Bool_DaMaRiS")){
        if(fPm->GetBooleanParameter("Ch/DaMaRiS/Bool_DaMaRiS")){
            ...
            G4cout << G4endl
                   ...
                   << "DrBreakMolecule registered as index "
                   << DrDefinitions::Instance()->GetBreakMoleculeIndex() << G4endl
            ...
            for (G4int i{0}; i < numberOfRepeats; i++) {
                ...
                if (goAhead != 0) {
                    ...
                    //@@@@ Writes out the integer number of molecules present in the
                    //@@@@ simulation  according to their type at the specified time
                    //@@@@ points. This is the preferred way of outputting data now.
                    std::ofstream rawDataFile("RawData"+std::to_string(altRunID)+".out",std::ios_base::app);
                    utilities.PrintTypeTable(rawDataFile);
                    rawDataFile << G4endl << G4endl;
                    rawDataFile.close();
