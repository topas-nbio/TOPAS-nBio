...
//    fGillespieWorld = std::pow(fSideLength, 3);
    fGillespieWorld = std::pow(3*um, 3);
...
void TsScoreWithIRTAndGillespieContinuous::Output() {
    ...
    G4double volume = std::pow(fSideLength, 3);
    G4double mass = volume * g/cm3;
    for (auto& NameAndTimeGValuePerTime: fGValuePerSpeciePerTime) {
        ...
        for (auto& TimeAndGValue:NameAndTimeGValuePerTime.second) {
            ...
            fEnergy = 1;
//            fEnergy  = fEnergyDep[tBin].second/fNbOfScoredEvents;
//            fEnergy = fTotalEnergyDeposit/fNbOfIRTRuns;

//            fGillespie->SetMolPerTime(fIRT->GetIRTConfiguration()->GetMoleculeIDs()[Name], fGValue);

            if (fNbOfScoredEvents > 1) {
                fGValueError = sqrt( (1.0/(fNumberOfThreads-1)) * (Gerror/fNumberOfThreads - fGValue*fGValue));
//                fEnergyError = sqrt( (1.0/(fNbOfScoredEvents-1)) * ((fEnergyDep[tBin].second)/fNbOfScoredEvents - fEnergy*fEnergy));
            }
            else {
                fGValueError = 1;
...
//            G4double Energy = fTotalEnergyDeposit;
...
//    G4cout<<fNumberOfRepetitions<<'\t'<<fNumberOfThreads<<'\t'<<fNumberOfRepetitions/fNumberOfThreads<<'\t'<<fNbOfIRTRuns<<G4endl;

//    G4cout<<nEvents<<" th event start"<<'\n';
...
//	G4cout<<"Run IRT: "<<fPulseTimeLimits.size()<<'\n';
