// Extra Class for TsDNAELSEPAElasticModel
// ********************************************************************
...
TsDNAELSEPAElasticModel::TsDNAELSEPAElasticModel(const G4ParticleDefinition*,
const G4String& nam) :
G4VEmModel(nam) 
{
  verboseLevel = 0;
...
TsDNAELSEPAElasticModel::~TsDNAELSEPAElasticModel()
{
  delete fpData_Au;
  delete fpData_H2O;
...
void TsDNAELSEPAElasticModel::Initialise(const G4ParticleDefinition* particle,
const G4DataVector& )
{
  if (verboseLevel > 3)
  G4cout << "Calling TsDNAELSEPAElasticModel::Initialise()" << G4endl;

  if (isInitialised) {return;}

  if(particle->GetParticleName() != "e-")
  {
    G4Exception("TsDNAELSEPAElasticModel::Initialise","em0001",
  ...
  for(G4int i=0; i<numOfCouples; ++i) 
  {
    ...
    if (nelm==1){// Protection: only for single element
      ...
      if (Z>0) 
      {
        ...
        if (path == nullptr)
        {
          G4Exception("TsDNAELSEPAElasticModel::Initialise","em0002",
        ...
        {
          G4Exception("TsDNAELSEPAElasticModel::Initialise","em0003",
      ...
      if(material == fpBaseWater && !fpData_H2O){
        ...
        {
          G4cout<<"TsDNAELSEPAElasticModel: low energy limit increased from "
        ...
        {
          G4cout<<"TsDNAELSEPAElasticModel: high energy limit decreased from "
        ...
        if (path == nullptr)
        {
          G4Exception("TsDNAELSEPAElasticModel::Initialise","em0004",
            FatalException,"G4LEDATA environment variable not set.");
          return;
        }
        ...
        if (!eDiffCrossSectionZ)
         G4Exception("TsDNAELSEPAElasticModel::Initialise","em0005",
         FatalException,
...
G4double TsDNAELSEPAElasticModel::CrossSectionPerVolume
 ...
 G4double)
{
  ...
  {
    G4cout <<
    "Calling CrossSectionPerVolume() of TsDNAELSEPAElasticModel"
    << G4endl;
  }
  ...
  if (nelm==1)  // Protection: only for single element
  {
    ...
    if (verboseLevel > 2)
    {
      ...
      G4cout << "=== TsDNAELSEPAElasticModel - XS INFO START" << G4endl;
      ...
      G4cout << "=== TsDNAELSEPAElasticModel - XS INFO END" << G4endl;
    }
  }
  else
  {
...
void TsDNAELSEPAElasticModel::SampleSecondaries(
      ...
      G4double)
{
  ...
  if (verboseLevel > 3){
    G4cout << 
    "Calling SampleSecondaries() of TsDNAELSEPAElasticModel"
    << G4endl;
  }
...
G4double TsDNAELSEPAElasticModel::Theta(G4int Z,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
//
G4double TsDNAELSEPAElasticModel::LogLinInterpolate(G4double e1,
G4double e2,
G4double e,
G4double xs1,
G4double xs2)
...
G4double TsDNAELSEPAElasticModel::LinLogInterpolate(G4double e1,
G4double e2,
G4double e,
G4double xs1,
G4double xs2)
...
G4double TsDNAELSEPAElasticModel::LinLinInterpolate(G4double e1,
G4double e2,
G4double e,
G4double xs1,
G4double xs2)
...
G4double TsDNAELSEPAElasticModel::LogLogInterpolate(G4double e1,
G4double e2,
G4double e,
G4double xs1,
G4double xs2)
...
G4double TsDNAELSEPAElasticModel::QuadInterpolator(
G4double cum11,
G4double cum12,
G4double cum21,
G4double cum22,
...
G4double TsDNAELSEPAElasticModel::RandomizeCosTheta(G4int Z, G4double k)
{
...
void TsDNAELSEPAElasticModel::SetKillBelowThreshold(G4double threshold)
{
  fkillBelowEnergy_Au = threshold;

  if (threshold < 10 * eV)
  {
    G4cout<< "*** WARNING : the TsDNAELSEPAElasticModel model is not "
