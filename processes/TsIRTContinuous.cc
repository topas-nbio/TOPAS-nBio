...
void TsIRTContinuous::Sampling(G4int pulse){
	if ( !fScorersInitialized )
		initializeScorers();
/*
	fXMin = -0.5 * fSideLength;
	fYMin = -0.5 * fSideLength;
	fZMin = -0.5 * fSideLength;

	fXMax = 0.5 * fSideLength;
	fYMax = 0.5 * fSideLength;
	fZMax = 0.5 * fSideLength;*/
...
//	while (fIRTValues.size() > 0) {
	while ((pulses > fEndPulse) || (irt < fTimeCut || globalTime < fTimeCut)) {
		...
		if(std::min(irt, globalTime) > fStartTime ||
				std::min(irt, globalTime) > fEndTime){
