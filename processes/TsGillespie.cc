...
//	fCubicVolume = (fCubicVolume / m3) * 1000;
	...
	for(auto it_reaction = fReactions.begin(); it_reaction != fReactions.end();){
		G4int Index  = it_reaction->first;
		if (fReactions[Index].reactionType != 6) {
			fReactions[Index].kobs *= (M*s) / (6.022e23 * fCubicVolume);
		}
		else {
			if (fReactions[Index].concentration != 0 && fReactions[Index].kobs != 0
					&& // first order reactions
				...
				fReactions[Index].concentration *= 6.022e23 * fCubicVolume / M;
				fReactions[Index].kobs *= (M*s) / (6.022e23 * fCubicVolume);
				...
				fReactions[Index].kobs *= fReactions[Index].concentration * (s);
			}
			else {
				fReactions[Index].kobs = fReactions[Index].scavengingCapacity * (s);
				fReactions[Index].concentration = 0;
...
/*
	...
	for(; test != fMoleculeCanReactWith.end(); ++test){
		...
		for(; test2 != test->second.end(); ++test2){
			G4int Index2 = (*test2);
			G4cout<<"Affected reactions: "<<Index2<<'\t'<<fMoleculeNames[fReactions[Index2].reactorA]<<'\t'<<fMoleculeNames[fReactions[Index2].reactorB]<<'\n';
		}
	}*/
}
	...
	if(fLastReactionIndex > 10000){ // full sampling
		for (auto& IndexAndReaction:fReactions) {
			...
			if (Type != 6) {
				...
				if (ReactA == ReactB) {
					if(ConA < 2) prop[i] = 0;
					else prop[i] = Kobs * ConA * (ConA-1); // * (0.5);
				}
				else {
...
//        G4cout<<"Input: "<<fCurrentTime<<'\t'<<fMoleculeNames[-index]<<" ("<<fConcentrations[-index]<<")"<<G4endl;
        return;
    }
...
//    G4cout<<"At rest: "<<fCurrentTime<<'\t'<<fMoleculeNames[ReactA]<<" ("<<fConcentrations[ReactA]<<") + "<<fMoleculeNames[ReactB]<<" ("<<fConcentrations[ReactB]<<") => ";
...
/*
	while(fCurrentTime <= fFinalTimeInSeconds) {
		auto step = Propensity();
		G4double StepPropensity = step.second;
		G4int StepIndex = step.first;
		...
		while(fCurrentTime > fTimeSteps[tBin] / s && tBin <= max_time){
			for(auto it = fMoleculeNames.begin(); it != fMoleculeNames.end(); ++it){
				G4String Mol = it->second;
				G4int index = it->first;
				fMoleculesAtTime[Mol][fTimeSteps[tBin]] = fConcentrations[index];
...
void TsGillespie::DiffuseAllMolecules() {
	...
	for(auto& IndexAndMolecule:fMoleculesFromIRT) {
		G4int Index = IndexAndMolecule.first;
		G4double dT = std::abs(fCurrentTime*s - fMoleculesFromIRT[Index].time);
		fIRT->GetIRTConfiguration()->Diffuse(fMoleculesFromIRT[Index],dT);
	}*/
}
...
*/
	...
	fMolPerTime[index] *= fCubicVolume * CLHEP::Avogadro;
	G4cout<<"converted to "<<fReverse[index] * fMolPerTime[index] * s<<" molecules/s in "<<fCubicVolume/mm3<<" mm3 "<<mass/kg<<" kg"<<G4endl;
