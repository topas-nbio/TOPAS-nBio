...
TsIRTConfiguration_revise::TsIRTConfiguration_revise(G4String name, TsParameterManager* pM)
...
TsIRTConfiguration_revise::~TsIRTConfiguration_revise()
{
	delete fUtils;
}


void TsIRTConfiguration_revise::AddMolecule(G4String name, G4double diffusionCoefficient,
...
G4bool TsIRTConfiguration_revise::MoleculeExists(G4String name) {
	if ( fMoleculesID.find(name) == fMoleculesID.end() ) return false;
	return true;
}


void TsIRTConfiguration_revise::Quit(const G4String& name, G4String message) {
	G4cerr << G4endl;
...
void TsIRTConfiguration_revise::QuitIfMoleculeNotFound(G4String mol) {
...
void TsIRTConfiguration_revise::Diffuse(TsMolecule& mol, G4double dt) {
...
void TsIRTConfiguration_revise::ResolveReactionParameters() {
...
void TsIRTConfiguration_revise::InsertReaction(G4String molA, G4String molB, std::vector<G4int> products,
...
void TsIRTConfiguration_revise::InsertBackgroundReaction(G4String A, G4String B, std::vector<G4int> p,
...
std::vector<G4ThreeVector> TsIRTConfiguration_revise::ResampleReactantsPosition(TsMolecule& molA, TsMolecule& molB, G4int index, G4double time) {
	// Position approach
...
std::vector<G4ThreeVector> TsIRTConfiguration_revise::GetBackgroundPositionOfProducts(TsMolecule molA, G4int index) {
	std::vector<G4ThreeVector> result;
...
G4double TsIRTConfiguration_revise::GetRCutOff(G4double tCutOff) {
...
G4double TsIRTConfiguration_revise::GetIndependentReactionTime(TsMolecule molA, TsMolecule molB, G4int indexOfReaction) {
...
G4int  TsIRTConfiguration_revise::ContactFirstOrderAndBackgroundReactions(TsMolecule molA) {
	G4int pdgA = molA.id;
...
std::pair<G4int, G4double> TsIRTConfiguration_revise::SampleIRTFirstOrderAndBackgroundReactions(TsMolecule molA ) {
	G4int pdgA = molA.id;
...
void TsIRTConfiguration_revise::SetTimeLimits(G4double lower, G4double upper) {
	fUpperTime = upper;
	fLowerTime = lower;
}


std::pair<G4String, G4String> TsIRTConfiguration_revise::GetReactants(G4int ReactionIndex) {
...
std::vector<G4String> TsIRTConfiguration_revise::GetProducts(G4int ReactionIndex) {
...
G4bool TsIRTConfiguration_revise::Inside(G4ThreeVector p ) {
...
void TsIRTConfiguration_revise::ScoreGvalue(std::vector<TsMolecule> &initialSpecies,
...
std::vector<G4double> TsIRTConfiguration_revise::GetH2SO4ComponentsConcentrationPH(G4double pH) {
...
std::vector<G4double> TsIRTConfiguration_revise::GetH2SO4ComponentsConcentrationP(G4double Concentration) {
	G4double Ka1 = pow(10,3);
	G4double Ka2 = pow(10,-1.987);
...
G4double TsIRTConfiguration_revise::GetIonicStrength(std::vector<G4double> Components) {
	G4double I = .5 * ((Components[0]) + (Components[1]) + (Components[2] * 4) + (Components[3]));
	return I;
}

G4double TsIRTConfiguration_revise::IonicRate(G4double IonicStrength, TsMolecularReaction Reaction) {
	G4double a = 0.15;
...
void TsIRTConfiguration_revise::AdjustReactionAndDiffusionRateForTemperature() {
...
void TsIRTConfiguration_revise::AdjustReactionRateForPH(G4String pHOrConcentration) {
...
G4double TsIRTConfiguration_revise::GetMoleculeRadius(G4int moleculeID) {
	return fMoleculesDefinition[moleculeID].radius;
}


G4int TsIRTConfiguration_revise::GetMoleculeCharge(G4int moleculeID) {
	return fMoleculesDefinition[moleculeID].charge;
}


std::vector<G4int> TsIRTConfiguration_revise::GetReactionProducts(G4int index) {
...
G4int TsIRTConfiguration_revise::GetReactionIndex(G4int pdgA, G4int pdgB) {
	for ( size_t u = 0; u < fMoleculeCanReactWith[pdgA].size(); u++ ) {
		if ( pdgB == fMoleculeCanReactWith[pdgA][u].first )
...
G4double TsIRTConfiguration_revise::GetOnsagerRadius(G4int molA, G4int molB) {
...
void TsIRTConfiguration_revise::PrintMoleculesInformation() {
...
void TsIRTConfiguration_revise::PrintReactionsInformation() {
