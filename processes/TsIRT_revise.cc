...
TsIRT_revise::TsIRT_revise(TsParameterManager* pM, G4String parmName): TsVIRTProcedure(pM,parmName){
	fReactionConf = new TsIRTConfiguration_revise(parmName, pM);
...
void TsIRT_revise::runIRT(G4double, G4double, G4double, G4bool) {
...
void TsIRT_revise::Sampling(){
	...
	for(auto it = fChemicalSpecies.begin(); it != fChemicalSpecies.end(); ++it){
		TsIRTConfiguration_revise::TsMolecule aMol = (*it).second;
...
void TsIRT_revise::AddMolecule(G4Track* aTrack, G4double time, G4int moleculeID, G4ThreeVector offset){
	...
	TsIRTConfiguration_revise::TsMolecule aMol = ConstructMolecule(aTrack, time, 0, G4ThreeVector());
...
TsIRTConfiguration_revise::TsMolecule TsIRT_revise::ConstructMolecule(G4Track* aTrack, G4double time, G4int moleculeID, G4ThreeVector offset) {
	G4int pdg = -1;
...
void TsIRT_revise::ConductReactions() {
	...
	while (fIRTValues.size() > 0) {
		...
		if ( irt < fTimeCut ) {
			...
			TsIRTConfiguration_revise::TsMolecularReaction binReaction = fReactionConf->GetReaction(indexOfReaction);
			...
			if ( !fIRTIsBackground[idx] ) {
				...
				if(fChemVerbosity > 0){
					G4cout<<"At time: "<<irt-fMinTime<<" ns"<<'\t'<<fMoleculesName[fChemicalSpecies[iM].id]
								<<" + "<<fMoleculesName[fChemicalSpecies[jM].id]<<" => ";
				}
				...
				if(fChemVerbosity > 0){
					...
					G4cout<<"At rest: "<<irt-fMinTime<<" ns"<<'\t'<<fMoleculesName[fChemicalSpecies[iM].id]<<" ("<<fChemicalSpecies[iM].id<<") "
			...
			for ( size_t u = 0; u < products.size(); u++ ) {
				TsIRTConfiguration_revise::TsMolecule aProd;
				aProd.id = products[u];
...
void TsIRT_revise::CleanIRTVariables() {
	fIRTIndex.clear();
	fIRTValues.clear();
	fIRTMolA.clear();
	fIRTMolB.clear();
...
G4bool TsIRT_revise::MoleculeExists(G4int Index) {
	std::unordered_map<G4int,TsIRTConfiguration_revise::TsMolecule>::iterator IT = fChemicalSpecies.find(Index);
...
void TsIRT_revise::RemoveMolecule(G4int Index) {
...
void TsIRT_revise::SetContainersForNextPulse() {;}
